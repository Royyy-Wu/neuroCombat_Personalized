...
def neuroCombat(dat,
    ...
    weights : a list of similarity matrices w/ shape = (samples in same batch, features) that denotes the similarity of each feature between samples, length of list = # samples
        - None by vanilla ComBat, include weights for Personalized)
        
    conv : convergence threshold for EM algorithm to find final adjustment parameter
    ...
    LS_dict = fit_LS_model_and_find_priors(s_data, design, weights, info_dict, mean_only)

    # find parametric adjustments
    if eb:
        if parametric:
            print('[neuroCombat] Finding parametric adjustments')
            gamma_star, delta_star = find_parametric_adjustments(s_data, weights, LS_dict, info_dict, mean_only, conv)
        else:
            print('[neuroCombat] Finding non-parametric adjustments')
            gamma_star, delta_star = find_non_parametric_adjustments(s_data, weights, LS_dict, info_dict, mean_only)
    else:
    ...
    bayes_data = adjust_data_final(s_data, design, gamma_star, delta_star, 
                                    s_mean, mod_mean, v_pool, info_dict,dat, weights)

    bayes_data = np.array(bayes_data)
    estimates = {'batches': info_dict['batch_levels'],'stand.data':s_data, 'var.p led': v_pool, 'stand.mean': s_mean, 'mod.mean': mod_mean, 'gamma.star': gamma_star, 'delta.star': delta_star}
...
# Functions to standardize the data
def standardize_across_features(X, design, info_dict):
    """
    Outputs
    ---------
    - s_data : standardized data
    - stand_mean : estimate of overall feature expression
    - var_pooled: variance of the entire pools
    - mod_mean: biological covariate factor
    """
...
def aprior(delta_hat):
    """
    parameter of inverse gamma prior
    """
    m = np.mean(delta_hat)
...
def bprior(delta_hat):
    ...
    m = np.mean(delta_hat)
    s2 = np.var(delta_hat,ddof=1)
    return (m*s2+m**3)/ float(s2)

def postmean(g_hat, g_bar, n, d_star, t2):
    """
    posterior estimate of location parameter of batch effect
    """
    return (t2*n*g_hat+d_star * g_bar) / (t2*n+d_star)

def postvar(sum2, n, a, b):
    """
    posterior estimate of scale parameter of batch effect
    """
...
def fit_LS_model_and_find_priors(s_data, design, weights, info_dict, mean_only):
    """
    Fit and find the prior of the
    Arguments
    ---------
    - s_data : standardized data
    - desgin: one-hot design matrix of batch and biological covariates
    - weights: similarity matrix of each feature among different samples
    - info_dict:
    
    Return
    ---------
    A dictionary of all prior parameters:
    - gamma_hat: batch i sample mean for each feature
    - delta_hat: batch i sample variance for each feature
    - gamma_bar: mean estimate of location effect
    - t2: variance estimate of location effect
    - a_piror: parameter estimate of scale effect with list length = #samples or #
    - b_prior: parameter estimate  of scale effect
    ...
    batch_info = info_dict['batch_info'] 
    n_sample = info_dict['n_sample']
    batch_design = design[:,:n_batch] #design matrix of batch effect
    
    # compuate batch sample mean gamma_hat
    if weights is not None:
        s_data_copy = copy.deepcopy(s_data)
        weights_copy = copy.deepcopy(weights)
        gamma_hat_list=[]
        for i, batch_idxs in enumerate(batch_info):
            for j in batch_idxs:
                num = np.average(s_data_copy[:,batch_idxs],weights = weights_copy[j].T,axis=1)
                num = num.reshape(num.shape[0],1)
                gamma_hat_list.append(num)
        gamma_hat = np.hstack(gamma_hat_list).T
        print('gamma_hat shape is',gamma_hat.shape)
    else:
        gamma_hat = np.dot(np.dot(la.inv(np.dot(batch_design.T, batch_design)), batch_design.T), s_data.T)
    
    # compuate batch sample variance delta_hat
    delta_hat = []
    for i, batch_idxs in enumerate(batch_info):
        if mean_only:
            delta_hat.append(np.repeat(1, s_data.shape[0]))
        else:
            if weights is not None: #delta_hat w/length = #samples
                for j in batch_idxs:
                    s_data_copy = copy.deepcopy(s_data)
                    weights_copy = copy.deepcopy(weights)
                    average = np.average(s_data_copy[:,batch_idxs],weights = weights_copy[j].T,axis=1)
                    unbiased_factor = np.sum(weights_copy[j].T,axis=1)-np.divide(np.sum(np.square(weights_copy[j].T),axis=1),np.sum(weights_copy[j].T,axis=1))
                    variance = np.average((s_data[:,batch_idxs]-average[:,None])**2, weights=weights_copy[j].T,axis=1)
                    unbiased_factor_tmp = copy.deepcopy(unbiased_factor)
                    unbiased_factor_tmp[unbiased_factor_tmp == 0] = 1
                    variance_unbiased = np.divide(variance,unbiased_factor_tmp)
                    delta_hat.append(variance_unbiased)
            else: #delta_hat w/shape = #batches
                delta_hat.append(np.var(s_data[:,batch_idxs],axis=1,ddof=1))
    
    delta_hat = list(map(convert_zeroes,delta_hat)) # Covert zero variance to baseline variance
    print('length of delta_hat is',len(delta_hat))
    print('shape of delta_hat[0] is',delta_hat[0].shape)
    print('delta_hat[0] is',delta_hat[0])
    ...
    print(len(a_prior))
    print(a_prior[0].shape)
    print(a_prior[0],b_prior[0])
...
def it_sol(sdat, g_hat, d_hat, g_bar, t2, a, b, weights, conv):
    sdat_copy = copy.deepcopy(sdat)
    if weights is None:
        n = (1 - np.isnan(sdat_copy)).sum(axis=1)
    g_old = g_hat.copy()
    d_old = d_hat.copy()
    ...
    while change > conv:
        if weights is None:
            ...
            sum2 = ((sdat_copy - np.dot(g_new.reshape((g_new.shape[0], 1)), np.ones((1, sdat_copy.shape[1])))) ** 2).sum(axis=1)
            d_new = postvar(sum2, n, a, b)
        else:
            g_new = (t2 * sdat_copy + d_old * g_bar) / (t2 + d_old)
            d_new = (b + 0.5*(sdat_copy-g_old)**2) / (a - 0.5)
...
def find_parametric_adjustments(s_data, weights, LS, info_dict, mean_only, conv):
    batch_info  = info_dict['batch_info'] 
    ref_level = info_dict['ref_level']
    ...
    for i, batch_idxs in enumerate(batch_info):
        if mean_only:
            ...
            if weights is not None:
                for j in batch_idxs:
#                    print(LS['gamma_hat'][j,:],LS['delta_hat'][j], LS['gamma_bar'][j], LS['t2'][j], LS['a_prior'][j], LS['b_prior'][j])
                    temp = it_sol(s_data[:,j], LS['gamma_hat'][j,:],
                                LS['delta_hat'][j], LS['gamma_bar'][j], LS['t2'][j],
                                LS['a_prior'][j], LS['b_prior'][j], weights, conv)
                    gamma_star.append(temp[0])
                    delta_star.append(temp[1])
            else:
                temp = it_sol(s_data[:,batch_idxs], LS['gamma_hat'][i],
                            LS['delta_hat'][i], LS['gamma_bar'][i], LS['t2'][i],
                            LS['a_prior'][i], LS['b_prior'][i], weights, conv)
                gamma_star.append(temp[0])
                delta_star.append(temp[1])

    gamma_star = np.array(gamma_star)   #gamma_star: len = #samples(personalized) or #batches (vanilla)
...
def find_non_parametric_adjustments(s_data, weights, LS, info_dict, mean_only):
    batch_info  = info_dict['batch_info'] 
    ref_level = info_dict['ref_level']
...
def adjust_data_final(s_data, design, gamma_star, delta_star, stand_mean, mod_mean, var_pooled, info_dict, dat, weights):
    sample_per_batch = info_dict['sample_per_batch']
    n_batch = info_dict['n_batch']
    n_sample = info_dict['n_sample']
    batch_info = info_dict['batch_info']
    ...
    bayesdata = copy.deepcopy(s_data)
    gamma_star = np.array(gamma_star)
    delta_star = np.array(delta_star)
    ...
    for j, batch_idxs in enumerate(batch_info):
        ...
        if weights is None:
            ...
            numer = np.array(bayesdata[:,batch_idxs] - gamma_star[batch_idxs,:].T)
